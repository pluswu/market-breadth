...
class USStockAgent(RestAgent):
    ...
    def _get_symbols(self, market):   
        url = 'https://api.nasdaq.com/api/screener/stocks?exchange=%s&tableonly=true&limit=25&offset=0&download=true' % market     
        response = self.do_request(url, None, method='GET')
        if response is not None:
            data = json.loads(response)
            #print(data)
            df = pd.json_normalize(data["data"]["rows"])
            #print(df.head(100))
            return df
        return None
    ...
    def _get_data(self, symbol, events):
        ...
        print("try_get_dataurl:{}".format(url))

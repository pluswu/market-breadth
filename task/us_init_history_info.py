...
list_sql = '''
            ...
            where total_cap > 10 or is_spx = 'Y' or is_ndx = 'Y' or is_dji = 'Y' or is_follow > 0;
           '''
...
for n in range(0, len(stk_codes), batch):
    ...
    #                    interval='1d')
    data = us.download(symbol_list=symbol_list, start=date.get_2year_ago(), end=date.get_end_day(),
                        interval='1d')
    for i in sub_codes:
